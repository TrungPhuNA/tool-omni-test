...
class ProxyService {
    async executeRequest(config) {
        ...
        if (preScript) {
            ...
            console.log('[Debug] Variables after script:', JSON.stringify(variables));
        }
        ...
        Object.keys(headers).forEach(key => {
            ...
            headers[key] = this.injectVariables(val, variables);
            if (val !== headers[key]) {
                console.log(`[Debug] Header ${key} changed: ${val} -> ${headers[key]}`);
            }
        });
    ...
    injectVariables(target, variables = {}) {
        ...
        // Đảm bảo variables là một object
        let vars = variables;
        ...
        // Sử dụng Regex quét toàn bộ văn bản để tìm {{ key }}
        return target.replace(/{{\s*(.+?)\s*}}/g, (match, key) => {
            const trimmedKey = key.trim();

            // 1. Thử tìm trực tiếp (nhanh nhất)
            if (vars[trimmedKey] !== undefined) return vars[trimmedKey];

            // 2. Thử tìm không phân biệt hoa thường
            const lowerKey = trimmedKey.toLowerCase();
            const foundKey = Object.keys(vars).find(k => k.toLowerCase() === lowerKey);

            if (foundKey !== undefined && vars[foundKey] !== undefined) {
                return vars[foundKey];
            }

            // Nếu không tìm thấy, giữ nguyên match ({{key}})
            return match;
        });
    }
}
